...
class mywindow(QWidget, Ui_Client):
    def __init__(self):
        ...
        # --- ИЗМЕНЕНИЕ НАЧАЛО (ПЕРЕКЛЮЧЕНИЕ ВИДЕО В ПОЛНОЭКРАННЫЙ РЕЖИМ) ---
    ...
    def resizeEvent(self, event):
        ...
        # Если видео в полноэкранном режиме, растягиваем его и выходим
    ...
    def keyPressEvent(self, event):
        # --- ИЗМЕНЕНИЕ НАЧАЛО (ВЫХОД ИЗ ПОЛНОЭКРАННОГО РЕЖИМА ВИДЕО) ---
        if event.key() == Qt.Key_Escape and self.video_is_fullscreen:
            self.toggle_video_fullscreen() # Выход из полноэкранного режима видео по клавише Escape
            return
        # --- ИЗМЕНЕНИЕ КОНЕЦ ---

        # Также обрабатываем выход из полноэкранного режима окна
        if event.key() == Qt.Key_Escape and self.isFullScreen():
            self.showNormal()
            return
            
        if(event.key() == Qt.Key_Up): self.on_btn_Up()
        elif(event.key() == Qt.Key_Left): self.on_btn_Left()
        elif(event.key() == Qt.Key_Down): self.on_btn_Down()
        elif(event.key() == Qt.Key_Right): self.on_btn_Right()
        elif(event.key() == Qt.Key_Home): self.on_btn_Home()
        elif(event.key() == Qt.Key_R): self.on_btn_Light()
        elif(event.key() == Qt.Key_T): self.on_btn_Track()
        elif(event.key() == Qt.Key_C): self.on_btn_Connect()
        elif(event.key() == Qt.Key_V): self.on_btn_video()
        if event.isAutoRepeat(): return
        if event.key() == Qt.Key_W: self.on_btn_ForWard(); self.Key_W=True
        elif event.key() == Qt.Key_S: self.on_btn_BackWard(); self.Key_S=True
        elif event.key() == Qt.Key_A: self.on_btn_Turn_Left(); self.Key_A=True
        elif event.key() == Qt.Key_D: self.on_btn_Turn_Right(); self.Key_D=True  
        elif event.key() == Qt.Key_Space: self.on_btn_Buzzer(); self.Key_Space=True
    ...
    def on_btn_Up(self): self.servo2 = min(180, self.servo2 + 10); self.VSlider_Servo2.setValue(self.servo2)
    def on_btn_Left(self): self.servo1 = max(0, self.servo1 - 10); self.HSlider_Servo1.setValue(self.servo1)
    def on_btn_Down(self): self.servo2 = max(80, self.servo2 - 10); self.VSlider_Servo2.setValue(self.servo2)
    def on_btn_Right(self): self.servo1 = min(180, self.servo1 + 10); self.HSlider_Servo1.setValue(self.servo1)
    def on_btn_Home(self): self.servo1, self.servo2 = 90, 90; self.HSlider_Servo1.setValue(self.servo1); self.VSlider_Servo2.setValue(self.servo2)
    def on_btn_Cam_Left(self): self.camera_angle = (self.camera_angle - 90) % 360
    def on_btn_Cam_Right(self): self.camera_angle = (self.camera_angle + 90) % 360
    def on_btn_Cam_Origin(self): self.camera_angle = 0
    def Change_Left_Right(self): self.servo1=self.HSlider_Servo1.value(); self.label_Servo1.setText(f"{self.servo1}"); self.send_servo_command()
    def Change_Up_Down(self): self.servo2=self.VSlider_Servo2.value(); self.label_Servo2.setText(f"{self.servo2}"); self.send_servo_command()
    def Fine_Tune_Left_Right(self): self.label_FineServo1.setText(str(self.HSlider_FineServo1.value())); self.send_servo_command()
    def Fine_Tune_Up_Down(self): self.label_FineServo2.setText(str(self.HSlider_FineServo2.value())); self.send_servo_command()
    ...
    def on_btn_Track(self):
        if self.commandFlag:
            self.trackFlag = 1 - self.trackFlag; self.lightFlag = 0; self.Light.setText("Light off"); self.Track.setText("Track on" if self.trackFlag else "Track off"); self.TCP.sendData(f"{cmd.CMD_TRACK}#{self.trackFlag}\n")
    def on_btn_Light(self):
        if self.commandFlag:
            self.lightFlag = 1 - self.lightFlag; self.trackFlag = 0; self.Track.setText("Track off"); self.Light.setText("Light on" if self.lightFlag else "Light off"); self.TCP.sendData(f"{cmd.CMD_LIGHT}#{self.lightFlag}\n")
